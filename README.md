...
Policy-based authorisation (deny-by-default)
...
Secrets management, dynamic DB creds, SSH CA



vault.zta.lab (own VM)
...
CMDB / source of truth for infrastructure



netbox.zta.lab (own VM)
...
Git server for GitOps workflows
...
SIEM, vulnerability scanning, brute-force detection
...
Automation orchestration (Policy Enforcement Point)



control.zta.lab (own VM)
...
Event-Driven Ansible (automated incident response)
...
3-switch fabric (spine + 2 leaf), VLANs, ACLs



central.zta.lab (containers)
...
central.zta.lab (RHEL container, 10.30.0.10)
...
Global Telemetry Platform (Flask)



central.zta.lab (RHEL container, 10.20.0.10)
...
Workshop automation and UI steps target Red Hat Ansible Automation Platform 2.6 (automation controller and Event-Driven Ansible controller). See the AAP 2.6 documentation.
...
Every AAP job checks OPA policy before execution
...
Vault issues DB credentials with minimum grants
...
Dynamic DB users expire; SSH certificates are time-bound
...
SSH requires a Vault-signed certificate — no static keys
...
OPA blocks all actions unless explicitly allowed
...
IdM groups control who can run which operations
...
SPIFFE/SPIRE proves the automation platform is legitimate
...
AAP Policy as Code blocks unauthorised launches
...
Arista ACLs isolate app and data tiers on the switch fabric
...
Wazuh watches every authentication attempt
...
EDA automatically revokes credentials on attack detection
...
Credential revocation limits attacker data access
...
Netbox validates infrastructure state for policy
...
Code push triggers automated, policy-governed pipelines
